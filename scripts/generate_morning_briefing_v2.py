#!/usr/bin/env python3
"""
Morning Briefing Generator V2 - Automatic First, Manual Fallback
Tries system-provided OAuth tokens, then falls back to manual input.
"""
...
def try_automatic_email_fetch():
    """Try to fetch emails automatically using system-provided tokens."""
    # Check if automatic fetching is available
    if not all([
        os.getenv('CAPY_GMAIL_ACCESS_TOKEN'),
        os.getenv('CAPY_GMAIL_REFRESH_TOKEN')
    ]):
        return None

    try:
        # Run the automatic fetcher
        result = subprocess.run(
            ['python3', 'scripts/fetch_emails_auto.py'],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def convert_api_data_to_briefing_format(api_data):
    """Convert Gmail API response to morning briefing format."""
    emails = api_data.get('emails', [])

    email_summary = f"{api_data.get('unread_count', 0)} unread emails"
    if len(emails) > 0:
...
def get_manual_input():
    """Get email data from manual input (environment or file)."""
...
def main():
    """Main function."""
    print("=" * 60)
    print("☀️  MORNING BRIEFING GENERATOR V2")
    print("=" * 60)

    # Step 1: Try automatic email fetching
    print("\n🔍 Checking for automatic email fetch...")
    api_data = try_automatic_email_fetch()

    if api_data:
        print("✓ Emails fetched automatically via system tokens!")
        email_data = convert_api_data_to_briefing_format(api_data)
        email_data['source'] = 'automatic'
    else:
        print("⚠️  Automatic fetch not available, using manual input")
